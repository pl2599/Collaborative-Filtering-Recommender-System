...
calc_weight <- function(data, method = "pearson", significance = F, variance = F) {
  ...
  weight_func <- function(rowA, rowB) {
    ...
    if (sum(joint_values) == 0) {
      ...
      if (method == 'pearson') {
        # if(significance & variance)
        # {
        #   if(sum(joint_values) < 50)
        #   {
        #     return((sum(joint_values)/50) * )
        #   }
        # }
        # if(signifiance)
        # {
        #   if(sum(joint_values) < 50)
        #   {
        #     return((sum(joint_values)/50) * cor(rowA[joint_values], 
        #                                         rowB[joint_values], method = 'pearson'))
        #   }
        # }
        # if(variance)
        # {
