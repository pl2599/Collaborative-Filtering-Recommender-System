...
```{r}
estimate_rating <- function(i, j, prob_class, prob_rating, assignment, movies_UI){
...
estimate_rating(1, 1, prob_class, prob_rating, assignment, movies_UI)
```


```{r}
predict_all <- function(prob_class, prob_rating, assignment, movies_UI){
  # fills in movies_UI matrix and returns full matrix
  for (i in 1:n_users){
    for (j in 1:n_items){
      if (is.na(movies_UI[i,j])){
        movies_UI[i,j] <- estimate_rating(i, j, prob_class, prob_rating, assignment, movies_UI)
      }
    }
  }
  return(movies_UI)
...
```{r}
system.time(predicted_UI <- predict_all(prob_class, prob_rating, assignment, movies_UI)) # will take around 8 min ?
```

```{r}
estimate_rating_movie <- function(j, prob_class, prob_rating, assignment, movies_UI){
...
predict_all_v2 <- function(prob_class, prob_rating, assignment, movies_UI){
  for (j in 1:n_items){
    movies_UI[,j] <- estimate_rating_movie(j, prob_class, prob_rating, assignment, movies_UI)
  }
  return(movies_UI)
}
```

Since we're using matrix operations, the prediction is much faster (1000 times faster):

```{r}
system.time(predicted_UI <- predict_all_v2(prob_class, prob_rating, assignment, movies_UI))
```
