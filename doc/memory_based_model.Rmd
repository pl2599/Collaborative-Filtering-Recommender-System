...
neighb_weights <- MS_sim_pearson[1, ]
...
save(MS_pred_msd, file = "MS_pred_msd.RData")
```

```{r}
MS_pred_msd[1:10,1:10]
```

```{r}
movie_pred_msd <- pred_matrix(movie_UI, movie_sim_msd)
save(movie_pred_msd, file = "movie_pred_msd.RData")
```

```{r}
movie_pred_msd[1:10,1:10]
```
...
```{r}
MS_acc_pearson <- test_acc_MS(MS_UI,MS_pred_pearson,MS_UI_test)
MS_acc_pearson
```
...
```{r}
MS_acc_spearman <- test_acc_MS(MS_UI,MS_pred_spearman,MS_UI_test)
MS_acc_spearman
```
...
```{r}
MS_acc_msd <- test_acc_MS(MS_UI,MS_pred_msd,MS_UI_test)
MS_acc_msd
```
...
```{r}
MS_acc_entropy <- test_acc_MS(MS_UI,MS_pred_entropy,MS_UI_test)
MS_acc_entropy
```
...
```{r}
MS_acc_vector <- test_acc_MS(MS_UI,MS_pred_vector,MS_UI_test)
MS_acc_vector
```
...
```{r}
MS_acc_simrank <- test_acc_MS(MS_UI,MS_pred_simrank,MS_UI_test)
MS_acc_simrank
```
